...
ls_l8  <- list.files(dir_l8, "tif$", full.names = T)
ls_l8  <- gsub("OutPut_", "", ls_l8)
dir_s1 <- paste0(dir_main, "/GIS_data/_for_share/Sentinel-1_VV_VH")
ls_s1  <- list.files(dir_s1, "tif$", full.names = T)
ls_s1  <- gsub("OutPut_", "", ls_s1)
# ref data
dir_rdata  <- paste0(dir_main, "/GIS_data/_for_share/manual_data")
name_rdata <- paste0(dir_rdata, "/visualInterpretation_data.Rdata")
load(name_rdata)
dt_ref <- dt_vis
...
`mapDisturbanceL8S1()` has 2 processing parts: RF model building and disturbance mapping. A simple implementation is to process both as follows. Please note that this function uses parallel processing and consume much memory and CPUs. Furrthermore, this process takes very long time.
